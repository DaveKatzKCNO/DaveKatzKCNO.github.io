 ...
 * 
 * 0. We learned in the last section that you can assign values to variables so that those values can be accessed and used throughout your code. 
 * Now, we learn that there are different TYPES of data and they all have their own rules and ways of behaving.
 * 
 ...
 * They don't hold, collect or aggregate other values, and operations on simple values return new simple values - they don't alter the og value.
 ...
 * 
 * When working with all datatypes, the expressions and the operations we perform on them can result in new or altered data, as is usually intended. 
 * 
 * The way the computer handles those operations differs between complex and primitive datatypes.
 * 
 * Primitive datatypes copy by value. They're dealing with immutable, simple bits of information stored in the compiler's memory. 
 * 
 * When you perform an action on a simple datatype, the computer finds where the original value is stored and physically copies it to a new place to be acted on.
 * 
 * That's why only new values are created and original values aren't altered. 
 * 
 * 
 * With more complex data, the computer still goes and finds where the values are stored in the memory, but it doesn't go through the effort of recreating that data in a new instance - it simply performs its actions on that original value and alters it.   
 */